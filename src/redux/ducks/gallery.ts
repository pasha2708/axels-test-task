...
export type deleteCommentActionTypes = {
	...
	payload: { data: DeleteCommentTypes };
};
...
export const recievePreview = (
	data: Array<RecievePreviewTypes>
): recievePreviewActionTypes => ({
	type: RECIEVE_PREVIEW,
	payload: { data },
});

export const recieveFullImg = (
	data: recieveFullImgTypes
): recieveFullImgActionTypes => ({
	type: RECIEVE_FULL_IMAGE,
	payload: { data },
});

export const settingLoadingFalse = (
	data: Boolean
): settingLoadingFalseActionTypes => ({
...
export function* fetchGetPreview(): SagaIterator {
	try {
		const previewImages: Array<RecievePreviewTypes> = yield call(
			fetchPreview
		);
...
export function* fetchFullImage(action: GetFullImageActionTypes): SagaIterator {
	try {
		const fullImage: recieveFullImgTypes = yield call(
			fetchImage,
			action.payload.data
		);
